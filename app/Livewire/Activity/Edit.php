...
class Edit extends Component
{
    ...
    {
        $this->activity = Activity::with(['activityType', 'semester'])->findOrFail($id);
        
        $this->name = $this->activity->name;
        $this->activity_type_id = $this->activity->activity_type_id;
        $this->start_date = $this->activity->start_date->format('Y-m-d');
        $this->end_date = $this->activity->end_date->format('Y-m-d');
        $this->semester_id = $this->activity->semester_id;
        $this->description = $this->activity->description ?? '';
        $this->color = $this->activity->color ?? '';
        
        // Load target grades
        $targetGrades = $this->activity->target_grades ?? [];
        $this->targetGradeX = in_array('X', $targetGrades);
        $this->targetGradeXI = in_array('XI', $targetGrades);
        $this->targetGradeXII = in_array('XII', $targetGrades);
        
        // Check if all grades or none (null)
        if ($this->activity->isForAllGrades()) {
    ...
    {
        // Auto-fill color from activity type if no custom color
        if ($value && !$this->color) {
    ...
    public function checkConflicts()
    {
        ...
        // Find overlapping activities (excluding current activity)
        $this->conflicts = Activity::where('id', '!=', $this->activity->id)
    ...
    public function update()
    {
        if (!auth()->user()->canManageActivities()) {
            $this->addError('error', 'Anda tidak memiliki akses untuk mengubah kegiatan.');
            return;
        }
        ...
        ActivityLog::createLog(
            action: 'update',
            modelType: 'Activity',
            modelId: $this->activity->id,
            description: "Mengubah kegiatan: {$this->activity->name}"
        );

        session()->flash('success', 'Kegiatan berhasil diperbarui!');

        // Simple redirect without parameters to avoid issues
    ...
    public function render()
    {
        ...
        $semesters = $this->activity->academicYear->semesters;
