content = r"""<div>
    ...
    @if(session()->has('success'))
        <div class="mb-4 rounded-lg bg-green-50 border border-green-200 p-4 text-sm text-green-800" role="alert">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-gray-500">Kerjakan asesmen dan kuis yang diberikan oleh sekolah maupun guru Anda</p>
    </div>
    ...
    <div class="mb-8">
        ...
        @if($quizAssessments->isEmpty())
            <div class="rounded-xl border border-dashed border-gray-200 bg-white p-6 text-center text-gray-400">
                <p class="text-sm">Belum ada kuis dari guru saat ini</p>
            </div>
        @else
            <div class="space-y-3">
                @foreach($quizAssessments as $quiz)
                    @php
                        $statusConfig = match($quiz->student_status) {
                            'upcoming'         => ['badge'=>'Akan Dimulai',     'bg'=>'bg-yellow-100', 'text'=>'text-yellow-800', 'icon'=>'Menunggu'],
                            'open'             => ['badge'=>'Bisa Dikerjakan',  'bg'=>'bg-green-100',  'text'=>'text-green-800',  'icon'=>''],
                            'in_progress'      => ['badge'=>'Sedang Dikerjakan','bg'=>'bg-blue-100',   'text'=>'text-blue-800',   'icon'=>''],
                            'submitted'        => ['badge'=>'Sudah Dikumpul',   'bg'=>'bg-purple-100', 'text'=>'text-purple-800', 'icon'=>''],
                            'closed_submitted' => ['badge'=>'Selesai',          'bg'=>'bg-gray-100',   'text'=>'text-gray-600',   'icon'=>''],
                            'closed_missed'    => ['badge'=>'Tidak Dikerjakan', 'bg'=>'bg-red-100',    'text'=>'text-red-700',    'icon'=>''],
                            default            => ['badge'=>$quiz->student_status,'bg'=>'bg-gray-100', 'text'=>'text-gray-600',   'icon'=>''],
                        };
                    @endphp
                    <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm flex flex-wrap items-center justify-between gap-3">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-3 text-xs text-gray-400 mt-1">
                                <span>{{ $quiz->start_date->translatedFormat('d M Y') }} {{ $quiz->start_time ? substr($quiz->start_time,0,5) : '' }} sampai {{ $quiz->end_date->translatedFormat('d M Y') }} {{ $quiz->end_time ? substr($quiz->end_time,0,5) : '' }}</span>
                            ...
                            @if($quiz->student_status === 'upcoming')
                                <div id="countdown-quiz-{{ $quiz->id }}"
                                    ...
                                    data-target="{{ $quiz->getOpenDatetime()->toIso8601String() }}"></div>
                            @endif
                        ...
                        <div class="shrink-0">
                            @if($quiz->student_status === 'upcoming')
                                <a href="{{ route('student.assessment.preview', $quiz->id) }}" wire:navigate
                                   class="inline-flex items-center gap-1.5 rounded-lg border border-yellow-300 bg-yellow-50 px-3 py-2 text-xs font-medium text-yellow-800 hover:bg-yellow-100 transition">
                                    Lihat Soal
                                </a>
                            ...
                            @elseif(in_array($quiz->student_status, ['submitted','closed_submitted']))
                                <a href="{{ route('student.assessment.result', $quiz->id) }}" wire:navigate
                                   class="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-700 hover:bg-gray-50 transition">
                                    Lihat Hasil
                                </a>
    ...
    {{-- SECTION 2: Asesmen Gaya Belajar (hanya tampil jika ada) --}}
...
path = r'C:\Users\DMCenter\Music\SPMB2\E-KALDIK\resources\views\livewire\student-assessment\index.blade.php'
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print('OK written', content.count('\n'), 'lines')
