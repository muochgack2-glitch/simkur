<div>
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @else
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 shadow-sm overflow-hidden cursor-not-allowed opacity-60">
                <div class="h-1.5 bg-gray-200"></div>
                <div class="p-5">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2.5 py-1 text-xs rounded-lg bg-gray-100 text-gray-500 font-semibold">{{ $course->subject->name ?? '-' }}</span>
                        <span class="text-xl">🔒</span>
                    </div>
                    <h3 class="font-bold text-gray-500 mb-1">{{ $course->title }}</h3>
                    <p class="text-xs text-gray-400">{{ $course->teacher->name ?? '' }}</p>
                    <p class="text-xs text-gray-400 mt-3 italic">Periode tidak aktif</p>
                </div>
            </div>
            @endif
            @endforeach
