...
<div x-data="{ modalImg: null, modalAnswer: null, modalName: '', showRevision: false }"
     @open-revision-modal.window="showRevision=true"
     @close-revision-modal.window="showRevision=false">
    ...
    <div x-show="modalAnswer" @click.self="modalAnswer=null" @keydown.escape.window="modalAnswer=null"
        style="display:none;position:fixed;inset:0;z-index:50;background:rgba(0,0,0,0.6);align-items:center;justify-content:center;padding:16px;"
        :style="modalAnswer ? 'display:flex' : 'display:none'">
    ...
    <div x-show="showRevision" @click.self="showRevision=false" @keydown.escape.window="showRevision=false"
        style="display:none;position:fixed;inset:0;z-index:50;background:rgba(0,0,0,0.6);align-items:center;justify-content:center;padding:16px;"
        :style="showRevision ? 'display:flex' : 'display:none'">
        <div style="background:white;border-radius:16px;box-shadow:0 25px 50px rgba(0,0,0,0.25);max-width:480px;width:100%;overflow:hidden;">
            <div style="padding:16px 20px;border-bottom:1px solid #fde68a;background:#fffbeb;display:flex;align-items:center;gap:12px;">
                <span style="font-size:22px;">✏️</span>
                <div>
                    <p style="font-weight:700;font-size:15px;color:#92400e;">Minta Kerjakan Ulang</p>
                    <p style="font-size:12px;color:#b45309;">Siswa akan diminta merevisi dan mengumpulkan ulang</p>
                </div>
                <button @click="showRevision=false" style="margin-left:auto;width:28px;height:28px;background:#fde68a;border-radius:50%;border:none;cursor:pointer;font-weight:bold;">&#10005;</button>
            </div>
            <div style="padding:20px;">
                ...
                <div style="display:flex;gap:10px;margin-top:16px;">
                    ...
                    <button wire:click="requestRevision" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="requestRevision">✏️ Kirim Permintaan</span>
                        <span wire:loading wire:target="requestRevision">Mengirim...</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="ag-mobile space-y-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 space-y-3"
            ...
            @else
            <p style="font-size:12px;color:#b45309;text-align:center;">Menunggu siswa mengerjakan ulang...</p>
            @endif
        </div>
        @empty
        ...
        <div style="overflow-x:auto;">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/30" style="{{ $sub->revision_requested ? 'background:#fffbeb;' : '' }}">
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4">
                            ...
                            @else
                            <span class="text-xs text-amber-600 italic">{{ $sub->revision_note ?: '—' }}</span>
                            @endif
                        </td>
                        <td class="py-3 px-4 text-center" style="white-space:nowrap;">
                            ...
                            <button wire:click="openRevisionModal({{ $sub->id }})" style="margin-left:4px;padding:6px 10px;font-size:11px;background:#f59e0b;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:600;">✏️ Ulang</button>
                            @else
