<div>
    ...
    @if(session('success'))
    <div class="mb-4 px-5 py-4 bg-green-50 border border-green-200 rounded-xl text-green-800 text-sm flex items-center gap-2">✅ {{ session('success') }}</div>
    @endif
    @if(session('error'))
    <div class="mb-4 px-5 py-4 bg-red-50 border border-red-200 rounded-xl text-red-800 text-sm flex items-center gap-2">❌ {{ session('error') }}</div>
    @endif
    ...
    <div class="mb-5 bg-green-50 border border-green-200 rounded-2xl overflow-hidden">
        <div class="px-5 py-4 flex items-center justify-between">
            ...
            @else
            <span class="text-xs text-amber-600 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-xl font-medium">⏳ Menunggu penilaian</span>
            @endif
        </div>
    ...
    <div class="mb-5 bg-blue-50 border border-blue-200 rounded-2xl p-4">
        ...
        <div class="flex items-start gap-2 flex-wrap sm:flex-nowrap">
            <div class="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-blue-200 text-xs text-blue-700 font-medium flex-shrink-0">
                <span class="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center text-[10px] font-black">1</span>
                Baca instruksi
            </div>
            <div class="text-blue-300 hidden sm:block mt-2">→</div>
            <div class="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-blue-200 text-xs text-blue-700 font-medium flex-shrink-0">
                <span class="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center text-[10px] font-black">2</span>
                Tulis jawaban
            </div>
            @if($assignment->allow_file_upload)
            <div class="text-blue-300 hidden sm:block mt-2">→</div>
            <div class="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-blue-200 text-xs text-blue-700 font-medium flex-shrink-0">
                <span class="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center text-[10px] font-black">3</span>
                Upload file
            </div>
            @endif
            <div class="text-blue-300 hidden sm:block mt-2">→</div>
            <div class="flex items-center gap-2 bg-white rounded-xl px-3 py-2 border border-blue-200 text-xs text-blue-700 font-medium flex-shrink-0">
                <span class="w-5 h-5 rounded-full bg-purple-600 text-white flex items-center justify-center text-[10px] font-black">{{ $assignment->allow_file_upload ? '4' : '3' }}</span>
                Kumpulkan
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border shadow-sm p-5 mb-5">
        <h3 class="font-bold text-gray-800 mb-3 flex items-center gap-2">📋 Instruksi Tugas</h3>
    ...
    <!-- Form Kumpulkan -->
    <form wire:submit.prevent="submit" class="bg-white rounded-2xl border shadow-sm overflow-hidden" onsubmit="return confirm('Yakin mau mengumpulkan tugas? Pastikan jawabanmu sudah benar.')">
        ...
        <div class="p-5 space-y-5">
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <label class="block text-sm font-semibold text-gray-700">Jawaban <span class="text-red-500">*</span></label>
                    <span class="text-xs text-gray-400" x-data x-text="($refs.answer?.value?.length || 0) + ' karakter'"></span>
                </div>
                <textarea wire:model="content" x-ref="answer" x-on:input="$el.nextElementSibling?.querySelector('span')?.innerText" rows="8"
            ...
            <div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-xl p-5 text-center hover:border-purple-400 transition-colors cursor-pointer">
        ...
        <div class="bg-gray-50 border-t px-5 py-4 flex justify-between items-center">
            ...
            <button type="submit" class="px-6 py-2.5 text-sm font-bold text-white bg-purple-600 hover:bg-purple-700 rounded-xl shadow transition-all flex items-center gap-2" wire:loading.attr="disabled">
