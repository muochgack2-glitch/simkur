<div>
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('pkl-learning.student.course', $assignment->course) }}" class="text-gray-500 hover:text-gray-700"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg></a>
        <div>
    ...
    @if(session('error'))
    <div class="mb-4 px-5 py-3 bg-red-50 border border-red-200 rounded-xl text-red-800 text-sm">{{ session('error') }}</div>
    @endif
    ...
    <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-5 mb-6 border border-gray-200 dark:border-gray-600">
        ...
        <p class="text-xs text-gray-500 mt-3">Deadline: {{ $assignment->deadline->translatedFormat('d M Y, H:i') }}
    ...
    <div class="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-5">
        <h3 class="font-semibold text-green-800 dark:text-green-300 mb-2">Sudah Dikumpulkan</h3>
        ...
        <p class="text-sm text-green-700 mt-2">Nilai: <strong>{{ $submission->score }}/{{ $assignment->max_score }}</strong></p>
        @if($submission->feedback)<p class="text-sm text-gray-600 mt-1">{{ $submission->feedback }}</p>@endif
        @endif
    </div>
    @endif
