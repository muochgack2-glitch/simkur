<div>
    ...
    <div class="mb-6 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-5">
        ...
        <p class="text-sm text-green-700 mt-1">Nilai: <strong>{{ $response->score }}/{{ $quiz->getTotalScore() }}</strong></p>
    </div>
    @endif
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
            ...
            <div class="space-y-2 ml-11">
                ...
                <label class="flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-all
                    {{ ($answers[$question->id] ?? '') === $opt ? 'bg-blue-50 border-blue-400 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-600 hover:bg-gray-50' }}
            ...
            <div class="flex gap-3 ml-11">
                ...
                <label class="flex items-center gap-2 px-5 py-3 rounded-xl border cursor-pointer transition-all
                    {{ ($answers[$question->id] ?? '') === $val ? 'bg-blue-50 border-blue-400' : 'border-gray-200 hover:bg-gray-50' }}
                    {{ $isFinished ? 'pointer-events-none' : '' }}">
    ...
    <div class="flex justify-between mt-6">
        ...
        <button wire:click="submitQuiz" wire:confirm="Yakin ingin mengumpulkan?" class="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl shadow-lg"><i class="fas fa-paper-plane mr-1"></i>Kumpulkan Kuis</button>
    </div>
    @endif
</div>
