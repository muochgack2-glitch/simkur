<div>
    ...
    @if(session('error'))
    <div class="mb-6 px-5 py-4 bg-red-50 border border-red-200 rounded-xl text-red-800 text-sm flex items-center gap-2">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors cursor-pointer" @click="expandedRow = expandedRow === {{ $v->id }} ? null : {{ $v->id }}">
                        ...
                        <td class="px-4 py-3 text-center" @click.stop>
                            <div class="flex items-center justify-center gap-1">
                                ...
                                @if($isAdmin)
                                <button
                                    wire:click="deleteVisit({{ $v->id }})"
                                    wire:confirm="Yakin hapus data kunjungan ini? Tindakan tidak dapat dibatalkan."
                                    class="px-3 py-1.5 border border-red-200 hover:bg-red-50 text-red-500 hover:text-red-700 rounded-lg text-xs font-medium transition-colors"
                                    title="Hapus">
                                    🗑️
                                </button>
                                @endif
